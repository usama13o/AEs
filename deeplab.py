...
class deeplab(pl.LightningModule):
        ...
        def __init__(self, num_classes, small=True, pretrained=True,proj_output_dim=None,pred_hidden_dim =None,num_predicted_clases=2, **kwargs):
                ...
                self.resnet_features = Atrous_ResNet_features(block, [3, 4, 23],kwargs['num_ch'], pretrained)
                self.resnet_features.conv1 =  self.conv1 = nn.Conv2d(kwargs['num_ch'], 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
                rates = [1, 6, 12, 18]
